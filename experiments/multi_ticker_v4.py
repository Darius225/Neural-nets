...
def save_prediction_plots(rows: list[dict], out_dir: str = "experiments/plots_v4") -> None:
    ...
    for r in rows:
        ...
        ax.set_title(
            f"{r['ticker']} v4  MAE={r['model'].mae:.2f}  "
...
def main() -> None:
    print(
        f"v4 multi-ticker pipeline: window={WINDOW_SIZE}d, 10 tech features, "
    ...
    print(
        f"model: {model.count_params():,} params, training on {len(split.X_train):,} samples...\n"
    )
