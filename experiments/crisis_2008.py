...
def save_prediction_plots(rows: list[dict], out_dir: str = "experiments/plots") -> None:
    ...
    for r in rows:
        ...
        ax.set_title(
            f"{r['ticker']} — actual vs CNN vs persistence  "
...
def print_per_ticker_table(rows: list[dict]) -> None:
    ...
    print(
        "note: persistence has no directional accuracy by construction "
...
def print_regime_shift_context(rows: list[dict]) -> None:
    ...
    for r in rows:
        ...
        outside = r["test_min$"] < r["train_min$"] or r["test_max$"] > r["train_max$"]
...
def main() -> None:
    ...
    for ticker in TICKERS:
        ...
        try:
            ...
            rows.append(row)
            print(
                f"  [{ticker}] done in {time.time() - t0:.1f}s "
